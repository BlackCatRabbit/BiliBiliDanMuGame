							...
							if (ProtocolVersion1 == 3)
							{
								...
								foreach (var msgItem in danmuJsonList)
								{
									...
									if ((string)msgItem["cmd"] == "DANMU_MSG")
									{
										...
										print("名称：" + msgItem["info"][2][1]+ "  Uid：" + msgItem["info"][2][0]+ "  弹幕：" + msgItem["info"][1]);
										Sprite sprite = await GetHeadImageByUID.GetHeadSprite(int.Parse(msgItem["info"][2][0].ToString()));

										//随机j建立人物模型
										GameManager._instance.CreatePlayer(int.Parse(msgItem["info"][2][0].ToString()), sprite);
